...
# Here we trying to split only (!) GAME

declare -a kmd_coins=(GAME)
for i in "${kmd_coins[@]}"
do
    echo -n [$i] 
    utxo=$($gamecredits_cli listunspent | grep .001 | wc -l)
    echo -n ' '$utxo
    if [ $utxo -eq 0 ]; then
	echo " Need funds!"
    else
		...
		curl --url "http://127.0.0.1:7776" --data "{\"coin\":\""${i}"\",\"agent\":\"iguana\",\"method\":\"splitfunds\",\"satoshis\":\"100000\",\"sendflag\":1,\"duplicates\":"${need}"}"
	    else
